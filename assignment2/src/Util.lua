...
function GenerateQuadsBricks(atlas)
    -- to get the locked box, change this to something the the balls/ powerups, then add the locked box in last
    ...
    for n =0, 2 do
        local i = 0
        for j = 0, 5 do
            quads[counter] = love.graphics.newQuad(x + i, y+ k, 32, 16, atlas:getDimensions())        
    ...
    quads[counter] = love.graphics.newQuad(x+160,y+k,32,16,atlas:getDimensions())
    -- counter should only go up to 22
    return quads
end
...
function GenerateQuadsPowerups(atlas)
    -- start at position 0 and 192 (16 bits * 12 prior images)
    local x = 0
    local y = 192
    local quads = {}
